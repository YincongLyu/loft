...
inline Target down_cast(Source *arg) {
    static_assert(
        !std::is_base_of<
            typename std::remove_pointer<Target>::type, Source>::value,
...
inline Target down_cast(Source &arg) {
    ...
    static_assert(
        !std::is_base_of<
            typename std::remove_reference<Target>::type, Source>::value,
        "Do not use down_cast for upcasts; use implicit_cast or nothing"
    );
    assert(
        dynamic_cast<typename std::remove_reference<Target>::type *>(&arg)
        != nullptr
    );
    return static_cast<Target>(arg);
}
...
inline InputIt FindTrimmedEnd(InputIt first, InputIt last, Pred &&pred) {
    return std::find_if_not(
               ...
               std::make_reverse_iterator(first), std::forward<Pred>(pred)
    )
        .base();
}
 ...
 */
template<class InputIt, class Pred>
inline std::pair<InputIt, InputIt>
FindTrimmedRange(InputIt first, InputIt last, Pred &&pred) {
