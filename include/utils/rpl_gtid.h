...
public:
  ...
  rpl_sidno get_max_sidno() const { return static_cast<rpl_sidno>(sidno_to_sid_map_.size()); }
...
private:
  ...
  static const unsigned char *sid_map_get_key(const unsigned char *ptr, size_t *length)
  {
  ...
  */
  std::unordered_map<rpl_sid, std::unique_ptr<Node>, binary_log::Hash_Uuid> sid_to_sidno_map_;
};
...
struct Gtid
{
  ...
  */
  ...
  bool equals(const Gtid &other) const { return sidno_ == other.sidno_ && gno_ == other.gno_; }
...
struct Gtid_specification
{
  ...
  */
  ...
  void set(const Gtid &gtid_param) { set(gtid_param.sidno_, gtid_param.gno_); }
  ...
  {
    return (type_ == other.type_ && (type_ != ASSIGNED_GTID || gtid_.equals(other.gtid_)));
  }
  ...
  */
  bool equals(const Gtid &other_gtid) const { return type_ == ASSIGNED_GTID && gtid_.equals(other_gtid); }
