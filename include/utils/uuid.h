...
struct Uuid
{
  ...
  void copy_from(const Uuid &data) { copy_from(static_cast<const unsigned char *>(data.bytes)); }

  /// Copies the given UUID object to this UUID.
  void copy_to(unsigned char *data) const { memcpy(data, bytes, BYTE_LENGTH); }

  /// Returns true if this UUID is equal the given UUID.
  bool equals(const Uuid &other) const { return memcmp(bytes, other.bytes, BYTE_LENGTH) == 0; }
...
struct Hash_Uuid
{
  ...
  {
    return std::hash<std::string>()(std::string(pointer_cast<const char *>(uuid.bytes), Uuid::BYTE_LENGTH));
  }
};
