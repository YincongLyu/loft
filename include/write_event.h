...
class Rows_event : public AbstractEvent{
  public :
    Rows_event(const Table_id &tid, unsigned long wid,uint16_t flag, Log_event_type type);
    ~Rows_event() override;
    ...
    */
    ...
    size_t get_data_size() override {
        return calculate_event_size();
    }
    // 模板函数定义需要与使用它的代码在同一编译单元中
    template<typename T>
    void data_to_binary(uint8_t* &buf, T *data, size_t &data_size, enum_field_types type, size_t length, size_t str_length,int precision, int frac) {
        switch (type)
        {
...
//                size_t  float_byte = 4;
                ...
                memcpy(buf + data_size, data, length);
                data_size += length;
                break;
            }
            ...
            case enum_field_types::MYSQL_TYPE_BLOB :{
                ...
                memcpy(buf + data_size, &str_length, length);
                data_size += length;
    ...
    template<typename T>
    void write_data_before(T *data, enum_field_types type, size_t length = 0, size_t str_length = 0,int precision = 0, int frac = 0){
        data_to_binary(m_rows_before_buf, data, data_size1, type, length, str_length, precision, frac);
    }

    template<typename T>
    void write_data_after(T *data, enum_field_types type, size_t length  = 0, size_t str_length = 0,int precision = 0, int frac = 0) {
