...
{
  if (state_ != State::RUNNING) {
    LOG_ERROR("[%s] cannot submit task. state=%d", pool_name_.c_str(), state_);
    return -1;
  }
...
void ThreadPoolExecutor::thread_func()
{
  ...
  if (ret != 0) {
    LOG_ERROR("[%s] set thread name failed", pool_name_.c_str());
  }
  ...
  if (iter == threads_.end()) {
    ...
    LOG_ERROR("[%s] cannot find thread state of %s", pool_name_.c_str(), oss.str().c_str());
    return;
  }
