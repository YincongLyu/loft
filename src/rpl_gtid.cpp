...
enum_return_status
Gtid_specification::parse(Sid_map *sid_map, const char *text) {
    // TODO
...
int Gtid_specification::to_string(const rpl_sid *sid, char *buf) const {
    switch (type_) {
        ...
        case UNDEFINED_GTID:
        case ASSIGNED_GTID:
            return gtid_.to_string(*sid, buf);
...
int Gtid_specification::to_string(const Sid_map *sid_map, char *buf) const {
    return to_string(
        type_ == ASSIGNED_GTID || type_ == UNDEFINED_GTID
            ? &sid_map->sidno_to_sid(gtid_.sidno_)
            : nullptr,
        buf
    );
}
