...
int Uuid::parse(const char *string, size_t len) { return parse(string, len, bytes); }

int Uuid::parse(const char *in_string, size_t len, const unsigned char *out_str)
{
  ...
  switch (len) {
    ...
    case TEXT_LENGTH:
      ...
      if (read_section(bytes_per_section[NUMBER_OF_SECTIONS - 1], &in_string, p_out_str)) {
        return 1;
      }
      break;
    default: return 1;
...
bool Uuid::read_section(int section_len, const char **section_str, const unsigned char **out_binary_str)
{
  const unsigned char **section_string = reinterpret_cast<const unsigned char **>(section_str);
...
bool Uuid::is_valid(const char *s, size_t len) { return parse(s, len, nullptr) == 0; }
